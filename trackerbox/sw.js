const CACHE_NAME = 'trackerbox-shell-v6-folder';
const APP_SHELL = ['./', './index.html', './trackerbox-app.js', './sw.js', './manifest.json', './icon-192.png', './icon-512.png'];
...
const fetchForWarmCache = (url) => {
  const isRemote = /^https?:\/\//i.test(url);
  return fetch(url, isRemote ? { mode: 'no-cors', cache: 'no-cache' } : { cache: 'no-cache' });
};
...
async function warmStartupAssets(cache) {
  await Promise.allSettled([
    ...APP_SHELL.map(async (url) => {
      try {
        const response = await fetchForWarmCache(url);
